...
public class ArcProgress extends ProgressBar {
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if(mStylePogress == STYLE_ARC){
            ...
            LinearGradient lg=new LinearGradient(0,getY()/2,getX(),getY()/2,Color.parseColor("#eeeeee"),Color.parseColor("#aaaaaa"), Shader.TileMode.MIRROR);
            mArcPaint.setShader(lg);
            canvas.drawArc(mArcRectf,90+angle,targetmDegree,false,mArcPaint);
            //绘制未完成部分
            LinearGradient lg2=new LinearGradient(0,getY()/2,getX(),getY()/2,Color.parseColor("#fb4300"),Color.parseColor("#ff7d00"), Shader.TileMode.MIRROR);
            mArcPaint.setShader(lg2);
